...
from elegantrl.textProcessor import w2vSumTextProcessor, berRnnSeperateTextProcessor
...
class Actor(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim, args= None):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(state_dim, mid_dim), nn.ReLU(),
                                 nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                                 nn.Linear(mid_dim, mid_dim), nn.ReLU(),
...
class ActorSACText(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim, env= None, args= None):
        super().__init__()
        if args["text_processor"] == "w2vSumProcessor":
            self.net_state = w2vSumTextProcessor(env, state_dim, mid_dim, args["use_price"], args["use_attention"], args["rnn_layers"], args["linear_layers"], args["dropout_prop"])
        elif args["text_processor"] == "bertRnnSeperateProcessor":
            self.net_state = berRnnSeperateTextProcessor(env, state_dim, mid_dim, 768, 300, args["bert_model"], args["use_price"], args["use_attention"], args["rnn_layers"], args["linear_layers"], args["dropout_prop"])
    ...
    def get_action(self, state):
        ...
        a_std = self.net_a_std(t_tmp).clamp(-20, 2).exp()
        #return torch.normal(a_avg, a_std).tanh()  # re-parameterize
        return torch.normal(a_avg, a_std)
...
class CriticTwinText(nn.Module):  # shared parameter
    def __init__(self, mid_dim, state_dim, action_dim, env= None, args= None):
        super().__init__()
        if args["text_processor"] == "w2vSumProcessor":
            self.net_sa_pre = w2vSumTextProcessor(env, state_dim, mid_dim, args["use_price"], args["use_attention"], args["rnn_layers"], args["linear_layers"], args["dropout_prop"])
            self.net_sa = nn.Sequential(nn.Linear(mid_dim + action_dim, mid_dim), nn.ReLU())
        elif args["text_processor"] == "bertRnnSeperateProcessor":
            self.net_sa_pre = berRnnSeperateTextProcessor(env, state_dim, mid_dim, 768, 300, args["bert_model"], args["use_price"], args["use_attention"], args["rnn_layers"], args["linear_layers"], args["dropout_prop"])
            self.net_sa = nn.Sequential(nn.Linear(mid_dim + action_dim, mid_dim), nn.ReLU())
        else:
            self.net_sa_pre = None
            self.net_sa = nn.Sequential(nn.Linear(state_dim + action_dim, mid_dim), nn.ReLU(),
                                        nn.Linear(mid_dim, mid_dim), nn.ReLU())  # concat(state, action)
        self.net_q1 = nn.Sequential(nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                                    nn.Linear(mid_dim, 1))  # q1 value
