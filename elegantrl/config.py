...
class Arguments:
    def __init__(self, agent, env=None, env_func=None, env_args=None, env_test= None, env_test_func= None, env_test_args= None):
        self.env = env  # the environment for training
        self.env_func = env_func  # env = env_func(*env_args)
        self.env_args = env_args  # env = env_func(*env_args)

        if env_test or env_test_func or env_test_args:
            self.env_test = env_test
            self.env_test_func = env_test_func
            self.env_test_args = env_test_args
        else:
            self.env_test = self.env
            self.env_test_func = self.env_func
            self.env_test_args = self.env_args
        ...
        self.eval_times1 = 2 ** 1  # number of times that get episode return in first
        self.eval_times2 = 2 ** 2  # number of times that get episode return in second
