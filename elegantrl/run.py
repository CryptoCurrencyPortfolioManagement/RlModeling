...
def train_evaluate_and_test(args: Arguments, run: None):
    torch.set_grad_enabled(False)
    args.init_before_training()
    gpu_id = args.learner_gpus
    ...
    '''init'''
    env = build_env(args.env, args.env_func, args.env_args)
    env_test = build_env(args.env_test,args.env_test_func, args.env_test_args)
    ...
    while if_train:
        ...
        epoch += 1
        trajectory = agent.explore_env(env, target_step)
        print("{}: created trajectory: {}".format(epoch, time.time()-start_time))
        start_time = time.time()
        steps, r_exp = buffer.update_buffer((trajectory,))
        print("{}: updated buffer: {}".format(epoch, time.time() - start_time))
        start_time = time.time()

        torch.set_grad_enabled(True)
        logging_tuple = agent.update_net(buffer)
        print("{}: update net: {}".format(epoch, time.time() - start_time))
        start_time = time.time()
        torch.set_grad_enabled(False)
        ...
        print("{}: evaluate agent: {}".format(epoch, time.time() - start_time))
        start_time = time.time()
        dont_break = not if_allow_break
        not_reached_goal = not if_reach_goal
        stop_dir_absent = not os.path.exists(f'{cwd}/stop')
        if_train = (dont_break or not_reached_goal) and epoch <= max_epochs and stop_dir_absent

    print(f'| UsedTime: {time.time() - evaluator.start_time:.0f} | SavedDir: {cwd}')
    if run:
        agent.save_or_load_agent(run.name + "_" + str(cwd), if_save=if_save_agent)
        buffer.save_or_load_history(run.name + "_" + str(cwd), if_save=True) if agent.if_off_policy else None
    else:
        agent.save_or_load_agent(cwd, if_save=if_save_agent)
        buffer.save_or_load_history(cwd, if_save=True) if agent.if_off_policy else None

    return agent.test_env(env_test)
