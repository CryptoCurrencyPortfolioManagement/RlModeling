...
from elegantrl.net import Actor, ActorSAC, ActorSACText, ActorFixSAC, CriticTwin, CriticTwinText, CriticREDq
...
class AgentBase:
    def __init__(self, net_dim: int, state_dim: int, action_dim: int, gpu_id=0, args=None, args_critic= None, args_actor= None):
        ...
        self.traj_list = [
            [[] for _ in range(4 if args.if_off_policy else 5)] for _ in range(self.env_num)
        ]

        act_class = getattr(self, 'act_class', None)
        cri_class = getattr(self, 'cri_class', None)
        self.act = act_class(net_dim, state_dim, action_dim, args.env, args_actor).to(self.device)
        self.cri = cri_class(net_dim, state_dim, action_dim, args.env, args_critic).to(self.device) if cri_class\
            else self.act
        ...
        self.cri_optim = torch.optim.Adam(self.cri.parameters(), args.learning_rate) if cri_class\
            else self.act_optim
        ...
        if self.env_num == 1:
            ...
            self.test_env = self.test_one_env
        else:
            self.explore_env = self.explore_vec_env
            self.test_env = self.test_vec_env
    ...
    def test_one_env(self, env) -> gym.Env:
        state = env.reset()
        done = False
        while not done:
            ten_s = torch.as_tensor(state, dtype=torch.float32).unsqueeze(0)
    ...
    def test_vec_env(self, env) -> list:
        last_done = torch.zeros(self.env_num, dtype=torch.int, device=self.device)
        ten_s = self.states
        step_i = 0
        ten_dones = torch.zeros(self.env_num, dtype=torch.int, device=self.device)

        while not any(ten_dones):
            ten_a = self.act.get_action(ten_s).detach()  # different
            ten_s_next, ten_rewards, ten_dones, _ = env.step(ten_a)  # different
            step_i += 1
            last_done[torch.where(ten_dones)[0]] = step_i  # behind `step_i+=1`
...
class AgentDDPG(AgentBase):
    def __init__(self, net_dim, state_dim, action_dim, gpu_id=0, args=None, args_critic= None, args_actor= None):
        self.if_off_policy = True
        self.act_class = getattr(self, 'act_class', ActorSAC)
        self.cri_class = getattr(self, 'cri_class', CriticTwin)
        super().__init__(net_dim, state_dim, action_dim, gpu_id, args, args_critic, args_actor)
...
class AgentDDPGText(AgentBase):
    def __init__(self, net_dim, state_dim, action_dim, gpu_id=0, args=None, args_critic= None, args_actor= None):
        if args:
            self.args_actor = [x for x in args.__dict__.keys() if "__argsActor" in x]
            self.args_actor = dict(zip([x.replace("__argsActor_", "") for x in self.args_actor], [args.__dict__[x] for x in self.args_actor]))
            self.args_critic = [x for x in args.__dict__.keys() if "__argsCritic" in x]
            self.args_critic = dict(zip([x.replace("__argsCritic_", "") for x in self.args_critic], [args.__dict__[x] for x in self.args_critic]))
        self.if_off_policy = True
        self.act_class = getattr(self, 'act_class', ActorSACText)
        self.cri_class = getattr(self, 'cri_class', CriticTwinText)
        super().__init__(net_dim, state_dim, action_dim, gpu_id, args, self.args_critic, self.args_actor)
        self.act.explore_noise = getattr(args, 'explore_noise', 0.1)  # set for `get_action()`
        ...
        for update_c in range(1, repeat_times):
            ...
            if update_a / update_c < update_a_rate:  # auto TTUR
                ...
                if update_h / update_c < update_h_rate:
                    update_h += 1
                    obj_h_term = self.get_obj_h_term(buf_state, buf_action, buf_r_norm, done_list)
                else:
                    obj_h_term = 0
                obj_a_std = self.criterion(self.act.get_a_log_std(state), avg_a_log_std)\
                    * self.lambda_a_log_std

                obj_actor = -(q_value_pg + logprob * alpha).mean() + obj_h_term + obj_a_std
                # obj_actor = -(q_value_pg + logprob * alpha).mean() + obj_a_std
    ...
    def update_net(self, buffer):
        ...
        for _ in range(int(1 + buf_len * self.repeat_times / self.batch_size)):
            indices = torch.randint(
                buf_len, size=(self.batch_size,), requires_grad=False, device=self.device
            )
